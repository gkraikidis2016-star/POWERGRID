let sb=null, currentUser=null, currentProfile=null, recovering=false;
...
$('resetBtn').onclick=async()=>{
  ...
  const {error}=await sb.auth.resetPasswordForEmail(email,{redirectTo:'https://gkraikidis2016-star.github.io/POWERGRID/'});
  msg(error?error.message:'Στάλθηκε email για αλλαγή κωδικού.');
};
$('savePasswordBtn').onclick=async()=>{
  const p=$('newPassword').value, p2=$('newPassword2').value;
  $('recoveryMsg').textContent='';
  if(p.length<6) return $('recoveryMsg').textContent='Ο κωδικός πρέπει να έχει τουλάχιστον 6 χαρακτήρες.';
  if(p!==p2) return $('recoveryMsg').textContent='Οι κωδικοί δεν ταιριάζουν.';
  const {error}=await sb.auth.updateUser({password:p});
  if(error) return $('recoveryMsg').textContent=error.message;
  recovering=false;
  $('recoveryBox').classList.add('hidden');
  $('newPassword').value=''; $('newPassword2').value='';
  $('authMsg').textContent='Ο κωδικός άλλαξε με επιτυχία.';
  const {data}=await sb.auth.getSession();
  if(data.session) await loadUser(data.session.user);
};
